...
// New: MenuBar, ScrollBar, StatusBar.  All existing widgets gain micro-animations.
// C++17. No deps beyond renderer.hpp / input.hpp / anim.hpp.
#pragma once
...
// ─────────────────────────────────────────────────────────────────────────────
using UIID = uint32_t;
inline UIID make_id(std::string_view s) {
...
public:
    ...
    bool button(std::string_view text, int w = 0) {
        ...
        // Blend base→hover→active colors via animated floats
        Color bg = lerp_color(
                       lerp_color(s_.accent, s_.accent_hov, hv),
                       s_.accent_act, pv);

        // Slight scale-down press effect via inset
        int inset = int(pv * 1.5f);
        Recti draw_r{rect.x+inset, rect.y+inset,
                     rect.w-inset*2, rect.h-inset*2};

        r_.fill_rounded_rect(draw_r, s_.radius, bg);
        r_.draw_text(draw_r.x+(draw_r.w-tw)/2,
                     draw_r.y+(draw_r.h-th)/2,
                     text, s_.accent_text, s_.font_scale);

        last_w_=w; last_h_=h; cy_+=h+s_.padding;
        return pressed;
    }

    // ── Outline button (animated) ─────────────────────────────────────────
    ...
    bool checkbox(std::string_view text, bool& value) {
        ...
        float fv = anim_.hover(id, value, dt(), 0.12f, 0.10f);  // fill anim
        float hv = anim_.press(id, hov && !value, dt(), 0.08f);

        Color bg = lerp_color(
                       lerp_color(s_.surface, s_.surface_hov, hv),
                       s_.accent, fv);
        r_.draw_panel(rbox, bg, s_.border);

        if (fv > 0.01f) {
            // Checkmark fades+draws in
            Color ck = {s_.accent_text.r, s_.accent_text.g, s_.accent_text.b,
                        uint8_t(fv*255)};
                ...
                float lo=0.f, float hi=1.f, int w=200)
    {
        ...
        int   krad = int(5.f + hv*2.f);   // knob grows on hover
    ...
    // ── Text field ────────────────────────────────────────────────────────
    bool text_field(std::string& buf, int w=200,
                    std::string_view placeholder={})
    {
        ...
        float fv = anim_.hover(id, focused, dt(), 0.1f, 0.15f);
        Color border_c = lerp_color(s_.border, s_.accent, fv);
        r_.draw_panel(rect,
                      lerp_color(s_.surface, s_.surface_hov, fv*0.5f),
                      border_c, focused?2:1);

        bool  empty   = buf.empty();
        Color tc      = empty ? s_.text_muted : s_.text;
    ...
    // ── Image box ─────────────────────────────────────────────────────────
    void image_box(const uint32_t* pixels, int img_w, int img_h,
                   int w=0, int h=0)
    {
        if (!pixels) return;
    ...
    // ─────────────────────────────────────────────────────────────────────
    // LIST BOX — animated selection highlight slide
    // ─────────────────────────────────────────────────────────────────────
                 ...
                 int& selected, int w=200, int visible_h=120)
    {
        ...
        if (in_.hovered(box))
            list_scroll_ -= in_.scroll_delta * item_h;
        list_scroll_ = std::clamp(list_scroll_, 0,
                                  std::max(0, total_h-visible_h));

        // ── Animated selection highlight ──────────────────────────────
        // Slides to the selected row using out_back easing
        if (selected >= 0) {
            float target_y = float(selected * item_h - list_scroll_ + box.y + 1);
            float sel_y    = anim_.select(pool_id, target_y, dt());
            Recti sel_r{box.x+1, int(sel_y), w-2, item_h};
            // Clip to box
            sel_r = intersect(sel_r, {box.x+1, box.y+1, w-2, visible_h-2});
            if (sel_r.h > 0)
                r_.fill_rounded_rect(sel_r, 3, s_.accent);
        }
        ...
        for (int i=0; i<int(items.size()); ++i) {
            ...
            UIID row_id = pool_id ^ uint32_t(i*31337);
            float hv = anim_.hover(row_id, hov && !sel, dt());

            // Non-selected rows get hover tint; selected row is covered by the slider
            if (!sel && hv > 0.01f)
                r_.fill_rect(row, {s_.surface_hov.r, s_.surface_hov.g,
                                   s_.surface_hov.b, uint8_t(hv*200)});

            Color fg = sel ? s_.accent_text : s_.text;
            r_.draw_text(cx_+1+s_.padding, iy+(item_h-th)/2,
                         items[i], fg, s_.font_scale);
    ...
    // ─────────────────────────────────────────────────────────────────────
    // DROPDOWN — animated open/close + slide-in items
    // ─────────────────────────────────────────────────────────────────────
    bool dropdown(const std::vector<std::string>& items,
                  int& selected, int w=200)
    {
        ...
        // Arrow animates rotation via 3-line trick
        int ax=cx_+w-s_.padding-6, ay=cy_+h/2-2;
        int flip = open ? 1 : 0;   // "▲" when open
        ...
        if (in_.clicked_in(head))
            dropdown_id_ = open ? 0 : id;

        // Animated panel: clips to ov * full_height
        if (ov > 0.01f) {
            int item_h  = h;
            ...
            for (int i=0; i<int(items.size()); ++i) {
                ...
                Color row_bg = rsel ? s_.accent : lerp_color(s_.surface,s_.surface_hov,rhv);
                r_.fill_rect(row, row_bg);
                r_.draw_text(cx_+s_.padding, py+i*item_h+(item_h-th)/2,
                             items[i], rsel?s_.accent_text:s_.text, s_.font_scale);
                if (rhov && in_.mouse_press[0]) {
                    selected=i; dropdown_id_=0; changed=true;
                }
            }
            r_.pop_clip();
    ...
    // ── Context menu (animated slide-down) ────────────────────────────────
    bool begin_context_menu() {
        if (in_.mouse_press[1]) {
            menu_open_=true; menu_x_=in_.mouse_x; menu_y_=in_.mouse_y; menu_off_=0;
        }
        return menu_open_;
    }
    ...
    bool context_menu_item(std::string_view text) {
        ...
        int visible_h = int(ov * float(mh));
        r_.push_clip({row.x, row.y, row.w, visible_h});

        bool hov = in_.hovered(row) && menu_open_;
        bool clk = in_.clicked_in(row);
        UIID rid = make_id(text) ^ 0xCCCC;
        float hv = anim_.hover(rid, hov, dt());
...
// ─────────────────────────────────────────────────────────────────────────────
// Usage:
//   MenuBar mb(r, in, style);
//   mb.begin(0, 0, win_width);
//   if (mb.menu("File")) {
//       if (mb.item("New"))   { ... }
//       if (mb.item("Open"))  { ... }
//       mb.separator();
...
public:
    ...
    // Returns true when this menu is open (user should populate items)
    bool menu(std::string_view label) {
        int tw = text_w(label);
        int bw = tw + 20;
        int bh = s_.menu_bar_h;
        Recti r{cur_x_, y_, bw, bh};
        UIID id = make_id(label);

        bool hov = in_.hovered(r);
        bool is_open = (open_id_ == id);

        float hv = anim_.hover(id, hov || is_open, float(in_.delta_time));
        Color bg = lerp_color(s_.bar_bg, s_.surface_hov, hv);
        if (is_open) bg = s_.accent;

        r_.fill_rect(r, bg);
        r_.draw_text(cur_x_+10, y_+(bh-text_h())/2,
                     label, is_open ? s_.accent_text : s_.text,
                     s_.font_scale);

        if (in_.clicked_in(r))
            pending_open_ = (is_open ? 0 : id);
    ...
    bool item(std::string_view label) {
        int th = text_h(), ih = th + s_.padding*2;
        Recti r{cur_item_x_+1, item_y_+item_offset_, cur_item_w_-2, ih};
        UIID id = make_id(label) ^ 0xABCD;

        bool hov = in_.hovered(r);
        bool clk = in_.clicked_in(r);
        float hv = anim_.hover(id, hov, float(in_.delta_time));

        Color bg = lerp_color(s_.surface, s_.surface_hov, hv);
        if (hov) bg = lerp_color(bg, s_.accent, hv*0.5f);
        r_.fill_rect(r, bg);
        Color tc = lerp_color(s_.text, s_.accent_text, hv*0.5f);
        r_.draw_text(cur_item_x_+1+s_.padding, item_y_+item_offset_+(ih-th)/2,
                     label, tc, s_.font_scale);

        item_offset_ += ih;
        // Extend dropdown rect height
    ...
    void separator() {
        ...
        r_.draw_line(cur_item_x_+4, sy, cur_item_x_+cur_item_w_-4, sy,
                     s_.bar_border, 1);
        item_offset_ += 10;
    }

    void end_menu() {}   // no-op; kept for symmetry
...
private:
    ...
    int text_w(std::string_view s) const {
        return r_.text_width(s, s_.font_scale);
    }
    int text_h() const { return r_.text_height(s_.font_scale); }
};
...
// Returns delta scroll in pixels if dragged.
//
// Usage (vertical):
//   ScrollBar sb(r, in, style);
//   int delta = sb.vertical(x, y, h, content_h, scroll_pos);
//   scroll_pos = std::clamp(scroll_pos + delta, 0, content_h - viewport_h);
...
public:
    ...
    static constexpr int W = 10;   // scrollbar width/height

    // Returns pixels scrolled this frame (positive = down)
    int vertical(int x, int y, int viewport_h,
                 int content_h, int scroll_pos)
    {
        ...
        float scroll_f = float(scroll_pos) / float(content_h - viewport_h);
        int   thumb_y  = y + int(scroll_f * float(viewport_h - thumb_h));
        ...
        r_.fill_rounded_rect(thumb, 3,
            lerp_color(s_.border, s_.accent, hv));
        ...
        if (drag_id_==id) {
            if (in_.mouse_down[0]) {
                ...
                float scale = float(content_h) / float(viewport_h);
                delta = int(dy * scale);
            } else {
                drag_id_=0;
            }
    ...
    int horizontal(int x, int y, int viewport_w,
                   int content_w, int scroll_pos)
    {
        ...
        if (drag_id_==id) {
            if (in_.mouse_down[0]) {
                ...
                float scale = float(content_w) / float(viewport_w);
                delta = int(dx * scale);
            } else {
                drag_id_=0;
            }
...
// STATUS BAR
// ─────────────────────────────────────────────────────────────────────────────
// Render at the bottom of the window.  Sections are left / right aligned.
//
// Usage:
//   StatusBar sb(r, in, style);
//   sb.begin(0, win_h - style.status_bar_h, win_w);
//   sb.text("Ready");
//   sb.text("Ln 42, Col 7");           // right side
//   sb.progress(0.72f, 80);            // inline progress strip
//   sb.color_dot(xm::colors::green);   // colored indicator dot
//   sb.end();
...
public:
    ...
    void color_dot(Color c, int radius=3) {
        int cy_ = y_ + s_.status_bar_h/2;
        r_.fill_circle(left_x_+radius, cy_, radius, c);
        left_x_ += radius*2 + 8;
    }

    // Vertical divider between sections
    void divider() {
        r_.draw_line(left_x_, y_+4, left_x_, y_+s_.status_bar_h-4,
                     s_.bar_border, 1);
        left_x_ += 12;
    }
